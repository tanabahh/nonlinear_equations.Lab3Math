...
class Root(Tk):
    def __init__(self):
        ...
        method_label = Label(child, text="Метод простых итараций и метод касательных (Ньютона)")
        method_label.pack()
        """
        ...
        if checkbox != 2:
            ...
            if (function_y == sin_y) and ((float(self.b.get()) < -6) or (float(self.b.get()) > 6)):
                ...
                method = SimpleIterationForSystems(float(self.a.get()), float(self.b.get()), float(self.accuracy.get()),
                                                   function, function_y,derived_x, derived_y)
                answer = method.get_answer()
                if answer >= 1000000:
                    if answer > 1000000:
                        answer = "Слишком большие чиселки в кубическом уравнении Промежуточный результат" + \
    ...
    def do_equation(self):
        ...
        method.find_answer()
        simple_method = SimpleIteration(float(self.a.get()), float(self.b.get()), float(self.accuracy.get()),
                                        function, derived_function)
        simple_method.find_answer()
        plt.title("График")
        plt.xlabel("x")
        plt.ylabel("y")
        plt.grid()
        ...
        plt.plot(x, [0 for i in x], "r--")
        if method.result:
            if simple_method.result:
                plt.scatter(simple_method.result, function(simple_method.result), color='blue', s=20, marker='o')
                plt.scatter(method.result, function(method.result), color='red', s=20, marker='o')
                self.answer_label.config(text="Метод Ньютона" + str(simple_method.result) + " Количество итерация " +
                                              str(simple_method.iteration)+"\nМетод простых итераций"
                                              + str(method.result) + " Количество итерация " + str(method.iteration))
            else:
                plt.scatter(method.result, function(method.result), color='red', s=20, marker='o')
                self.answer_label.config(text="Метод Ньютона" + str(method.result) + " Количество итерация " +
                                              str(method.iteration)+"\nДля метода простых итераций слишком большой "
                                                                    "интервал, возьмите расстояние не больше 0.2, "
                                                                    "либо на этом интервале нет корней")
        else:
            if simple_method.result:
                plt.scatter(simple_method.result, function(simple_method.result), color='red', s=20, marker='o')
                self.answer_label.config(text="Метод простых итераций" + str(simple_method.result)
                                              + " Количество итерация " + str(simple_method.iteration) +
                                              "\nДля метода Ньютона не выполнено условие сходимости")
            else:
                self.answer_label.config(text="Для метода Ньютона не выполнено условие сходимости. "
                                              "\n Для метода простых итераций слишком большой интервал, "
                                              "возьмите расстояние не больше 0.2 "
                                              "либо на этом интервале нет корней")
        plt.show()
        ...
        if self.var_for_methods.get() == 0:
            ...
            method.find_answer()
            if method.result:
                self.answer_label.config(text=str(method.result)+" Количество итерация "+str(method.iteration))
                plt.title("График")
                plt.xlabel("x")
                plt.ylabel("y")
                plt.grid()
